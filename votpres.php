...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['candidate_id'])) {
    ...
    $position = "president"; // Set the position you're voting for
    $user_id = $_SESSION['user_id'];
    
    // Check if user has already voted for this position
    $check_vote = $db->prepare("SELECT id FROM president3 WHERE user_id = ? AND position = ?");
    ...
    $check = $db->prepare("SELECT id FROM president WHERE id = ?");
    $check->bind_param("i", $candidate_id);
    $check->execute();
    
    if ($check->get_result()->num_rows > 0) {
        // Record that this user has voted
        $record_vote = $db->prepare("INSERT INTO president3 (user_id, candidate_id, position) VALUES (?, ?, ?)");
        $record_vote->bind_param("iis", $user_id, $candidate_id, $position);
        $record_vote->execute();
        
        // Check if vote record already exists
        $vote_check = $db->prepare("SELECT id FROM president2 WHERE candidate_id = ? AND position = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $db->prepare("UPDATE president2 SET total_votes = total_votes + 1 WHERE candidate_id = ? AND position = ?");
            $stmt->bind_param("is", $candidate_id, $position);
        } else {
            // Insert new vote record
            $stmt = $db->prepare("INSERT INTO president2 (candidate_id, position, total_votes) VALUES (?, ?, 1)");
            $stmt->bind_param("is", $candidate_id, $position);
        }
        ...
        $count_query = $db->prepare("SELECT total_votes FROM president2 WHERE candidate_id = ? AND position = ?");
...
<body>
    <div class="container">
        ...
        <p class="subtitle">Which candidate should be our next president?</p>
